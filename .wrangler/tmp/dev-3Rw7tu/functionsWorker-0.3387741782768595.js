...
// .wrangler/tmp/bundle-6rVBE8/checked-fetch.js
...
var require_checked_fetch2 = __commonJS2({
  "../.wrangler/tmp/bundle-BXmqYj/checked-fetch.js"() {
    var urls = /* @__PURE__ */ new Set();
    function checkURL(request, init) {
      const url = request instanceof URL ? request : new URL(
        (typeof request === "string" ? new Request(request, init) : request).url
...
var hashedAdminPass = await hashPassword("admin123");
users.push({
  id: userIdCounter++,
  name: "Admin",
  username: "admin",
...
async function onRequest({ request, env, params }) {
  ...
  try {
    if (path === "/api/register" && method === "POST") {
      ...
      return new Response(JSON.stringify({ message: "User registered successfully", userId: newUser.id }), { headers });
    }
    ...
    if (path === "/api/verificar-admin" && method === "POST") {
      ...
      return new Response(JSON.stringify({
        success: true,
        user: { id: adminUser.id, username: adminUser.username, userType: "administrador" }
      }), { headers });
    }
    if (path === "/api/health") {
      return new Response(JSON.stringify({ status: "ok", timestamp: (/* @__PURE__ */ new Date()).toISOString() }), { headers });
    }
    if (path === "/enviar-reserva" && method === "POST") {
      try {
        const body = await request.json();
        console.log("Reserva received:", body);
        return new Response(JSON.stringify({
          success: true,
          message: "Reserva recibida"
        }), { headers });
      } catch (e) {
        return new Response(JSON.stringify({ error: "Error processing request" }), { status: 500, headers });
      }
    }
...
//# sourceMappingURL=functionsWorker-0.3387741782768595.js.map
